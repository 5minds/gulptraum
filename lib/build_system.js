...
class BuildSystem {
  ...
  _initializeSystemTasks() {
    systemTasks.generate(this.gulp, this.config);
  }
  ...
  _registerTasksAfterSteps() {
    ...
    this._registerConventionalTask('doc');
  ...
  _getBuildTasksForConventionalTask(taskName) {
    ...
    const tasksRegistered = tasks.filter((task) => {
      return this.isTaskRegistered(task);
    });
