...
function generate(gulp, config) {
  ...
  const setupOutputFolderPath = path.relative(config.paths.root, config.paths.setup);

  gulp.task('setup-dev-sass-clean', () => {
  ...
  gulp.task('setup-dev-sass-build', () => {

    pathsToCopy.forEach((pathToCopy) => {

      const sourcePath = path.resolve(__dirname, pathToCopy);
      const targetPath = path.resolve(setupOutputFolderPath, pathToCopy);
  ...
  gulp.task('setup-dev-sass', (callback) => {
